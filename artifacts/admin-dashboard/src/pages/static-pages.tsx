  ...
  function handleSave() {
    ...
    if (editing) {
      updateMutation.mutate(
        { id: editing.id, data: { title: form.title, slug: slugClean, content: form.content, isPublished: form.isPublished } },
        {
          onSuccess: () => { toast({ title: "Updated" }); setDialogOpen(false); queryClient.invalidateQueries({ queryKey: ["/api/admin/pages"] }); },
          onError: (e: unknown) => {
            const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to update page.";
      ...
      createMutation.mutate(
        { data: { title: form.title, slug: slugClean, content: form.content, isPublished: form.isPublished } },
        {
          onSuccess: () => { toast({ title: "Created" }); setDialogOpen(false); queryClient.invalidateQueries({ queryKey: ["/api/admin/pages"] }); },
          onError: (e: unknown) => {
            const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message ?? "Failed to create page.";
            toast({ title: "Error", description: msg, variant: "destructive" });
          },
        }
      );
    ...
    <div className="space-y-6">
          ...
          <div className="flex gap-2 mt-4 flex-wrap justify-center">
            {SLUG_SUGGESTIONS.slice(0, 2).map((slug) => (
              <Button key={slug} size="sm" variant="outline" onClick={() => {
                ...
                setForm({ title: label, slug, content: "", isPublished: false });
                setDialogOpen(true);
              }}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>{editing ? `Edit: ${editing.title}` : "New Static Page"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2 overflow-y-auto flex-1">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Enter page content (plain text or markdown)…"
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove the page. Mobile users will no longer be able to view it.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
